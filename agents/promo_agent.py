...
SCRAPER_API_KEY = os.environ.get("SCRAPER_API_KEY", "b613088dc6a3ed7ce8fea2d765ea7ced")
...
def scrape_with_playwright(url):
    ...
    if not result or len(result) < 200:
        log("info", f"Direct failed, trying ScraperAPI for {url}")
...
def _scrape_direct(url):
    try:
        ...
        log("warn", f"Direct scrape failed for {url}: {e}")
        return ""
